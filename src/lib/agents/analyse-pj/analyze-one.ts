...
const DETECTED_TYPES: ReadonlySet<DetectedType> = new Set<DetectedType>([
...
export async function analyzeOneAttachment(args: {
  ...
  const mimeLower = attachment.mime_type.toLowerCase();
  const isPdf = mimeLower === 'application/pdf';
  const isImage = ACCEPTED_IMAGE_MIME.has(mimeLower);
  if (!isPdf && !isImage) {
  ...
  const mediaBlock = isPdf
    ...
    : {
        ...
        source: {
          type: 'base64' as const,
          media_type: (mimeLower === 'image/jpg' ? 'image/jpeg' : mimeLower) as
            | 'image/jpeg'
            | 'image/png'
            | 'image/webp'
            | 'image/gif',
  ...
  return runAgent<LlmAnalysisResult>({
    ...
    run: async () => {
        ...
        messages: [
          {
            ...
            content: [
              mediaBlock,
              { type: 'text', text: 'Analyse cette pièce jointe et réponds en JSON selon le schéma.' },
            ],
          },
        ],
      });
      ...
      try {
        ...
        throw new Error('JSON parse: ' + err + ' (preview: ' + rawText.slice(0, 200) + ')');
      }
