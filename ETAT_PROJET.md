# État du projet FoxO — snapshot 2026-08-01 — CHANTIER MODE APPEL COMPLET (phases 1-5, PRs #155 #156 #157 #158 #159)

ÉTAT GIT : main = e0198af (merge PR #159). En prod via Vercel. Vérifier le git log live en début de session.

CHANTIER MODE APPEL (secrétaire au téléphone) — CLOS, 5 phases, toutes validées end-to-end en préversion puis mergées :
- Phase 1 (PR #155, merge ab56428) : recherche globale admin. Migration occupants.telephone_digits (colonne générée chiffres seuls + index partiel). Route GET /api/admin/search (5 catégories en parallèle : interventions/occupants/clients/acps/organisations, ilike, limite 8/cat, inclut les clôturées). TopbarSearch (topbar sticky du layout admin, Ctrl+K, debounce 300ms, AbortController). Backlog : navigation clavier dans les résultats (flèches/Entrée) non faite.
- Phase 2 (PR #156, merge 987e398) : fiche client 360° sur /admin/clients/[id]. JournalPanel EXTRAIT du monolithe InterventionsClient → src/components/admin/JournalPanel.tsx. Helper rapport historique extrait → src/lib/rapport-historique.ts (portail repointé dessus). Chargeur client360.ts (interventions par acp_id si client ACP sinon client_id ; impayé = règle canonique réimplémentée filtrée client, TZ Bruxelles — PAS computeRelancesDues ; dernier mail = MAX(intervention_mails.date, mails_analyses.recu_le)). Type Client.acp_id ajouté. Limites assumées : factures liées par organisation_id non comptées dans l'impayé ; JournalPanel mono-intervention (dernier dossier) ; limite 100 interventions.
- Phase 3 (PR #157, merge 7a756ef — PR créée/mergée par Foxo via GitHub UI) : bouton Nouveau RDV pré-rempli sur la fiche. Prop optionnelle initial sur ColdInterventionModal (rétro-compatible, soumission createInterventionCold intacte) ; onCreated enrichi d'un argument optionnel { intervention_id, ref } ; buildRdvInitial (rdv-initial.ts) ; occupants hérités posés conf='en_attente'.
- Phase 4 (PR #158, merge 7bcbd61) : notes d'appel horodatées. Table notes_appel (client_id NOT NULL CASCADE, intervention_id SET NULL, RLS ENABLE+FORCE SANS policy = service-role uniquement ; index notes_appel_client_idx + notes_appel_intervention_idx — noms prod alignés dans la migration par 9d52faa). API GET/POST /api/admin/clients/[id]/notes (vérif appartenance dossier au POST, created_by = email session, événement timeline 'note_appel' best-effort). NotesAppelSection entre bandeau Situation et ClientForm.
- Phase 5 (PR #159, merge e0198af) : résumé IA à la demande. Agent resume_situation (kind utility) — CHECK agent_logs à 13 valeurs, déclarations TS aux 3+1 endroits (l'union AgentName de types/database.ts:~762 N'EST PAS morte : consommée par AgentLog + /admin/observabilite — resume_situation ajouté, dette des 9 autres valeurs manquantes laissée en l'état). Colonnes clients.resume_ia / resume_ia_genere_le. Route POST .../resume : cache par fraîcheur (lastActivity = max interventions/notes/mails/factures), force:true = régénération, contexte texte plafonné ~8000c (rognage vieux d'abord), prompt anti-invention, sortie texte brut. ResumeIASection sous le bandeau Situation.

MIGRATIONS SQL du chantier (toutes appliquées en prod AVANT le code, fichiers dans db/migrations/) : 2026-07-30_occupants_telephone_digits.sql, 2026-07-31_notes_appel.sql, 2026-07-31_resume_situation.sql.

BACKLOG issu du chantier : navigation clavier TopbarSearch ; indicateur impayé dans le dropdown de recherche (refusé : alourdirait la route appelée à chaque frappe) ; factures organisation_id dans le calcul d'impayé client ; pagination notes_appel au-delà de 50 ; désynchronisation historique ALL_AGENT_NAMES/AGENT_KIND_BY_NAME (extraction_cas, extraction_achat manquants) et union AgentName de types/database.ts (9 valeurs manquantes) ; bug préexistant recherche morte du drawer « Liens » (InterventionsClient.tsx:~4538, fetch jeté).

INVARIANTS INCHANGÉS : crons mails toujours fermés ; repo > doc ; migration repo != base (vérifier en SQL) ; tsc + hook pre-push verts ; merge commit jamais squash + supprimer branche ; SQL via Supabase uniquement ; préversion Vercel = PROD (sandbox 2026-000) ; createAdminClient pour écritures serveur ; agents canoniques via runAgent ; dispatch.ts = assemblage unique ; photos non numérotées ; Facturation = dernier chantier.

INTENDANCE : ré-uploader ce ETAT_PROJET.md dans la knowledge du projet (même URL raw) après ce commit. Supprimer côté GitHub les branches mergées : feat/mode-appel-recherche, feat/mode-appel-fiche-360, feat/mode-appel-rdv-prerempli, feat/mode-appel-notes, feat/mode-appel-resume-ia.
...
1. ~~Lot Signature visuelle~~ — CORRIGÉ (2e audit 2026-07-02) : DÉJÀ LIVRÉ. feat/signature-pdf mergée le 2026-06-20 (PR #114, merge 912ed24, branche supprimée) : refonte visuelle PDF en prod, validée visuellement par Foxo ; décisions tranchées (couverture claire, « Fait à » = ville de l'ACP, repli Kortenberg) ; clôture volontairement réduite à « Fait à [ville], le [date] » (pas de bloc signature/cachet — choix de design assumé).
1-bis. NOUVEAU (issu du même audit) — lot optionnel « Signature mails sortants » : aucun bloc signature société partagé n'existe ; canaux SANS signature : réponse manuelle page Mails (sendMailReply — le corps part tel que tapé), notif message partenaire (notify-partner-message.ts), emails RDV (email/rdv.ts) ; canaux avec en-tête mais sans pied société : notifications de statut + confirmations occupants (email/notifications.ts), rappel J-1 (email/rappel-j1.ts). Reste à faire SI le besoin se confirme à l'usage : factoriser un bloc signature société (± logo) et l'appliquer aux canaux manquants. Non bloquant, non planifié.
