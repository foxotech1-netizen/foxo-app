...
export function ConfirmActions({
      ...
      <div>
        <div className="flex flex-col gap-2.5">
          <button
            ...
            className="bg-[var(--color-ok)] text-[var(--color-cream)] py-3.5 rounded-xl font-bold text-[14px] disabled:opacity-50 active:opacity-80 inline-flex items-center justify-center gap-2"
          >
          ...
          <button
            ...
            disabled={pending}
            className="bg-[var(--color-terra)] text-[var(--color-cream)] py-3.5 rounded-xl font-bold text-[14px] disabled:opacity-50 active:opacity-80 inline-flex items-center justify-center gap-2"
          >
          ...
          <button
            ...
            disabled={pending}
            className="bg-[var(--color-navy)] text-[var(--color-cream)] py-3.5 rounded-xl font-bold text-[14px] disabled:opacity-50 active:opacity-80 inline-flex items-center justify-center gap-2"
          >
            <RefreshCw size={16} /> Proposer un autre créneau
          </button>
        </div>
        {pending && (
          <p className="text-[11px] text-[var(--color-ink-mid)] text-center mt-2">Enregistrement…</p>
        )}
        {error && (
          <div className="text-[12px] text-[var(--color-terra)] bg-[var(--color-terra-light)] border border-[var(--color-terra-mid)] rounded-md px-3 py-2 mt-2">
            {error}
          </div>
        )}
      </div>
  ...
  if (mode === 'decline') {
    return (
      <div className="bg-[var(--color-sand-mid)] border border-[var(--color-sand-border)] rounded-xl p-3.5 space-y-2.5">
        <div className="text-[12px] font-bold text-[var(--color-ink)] inline-flex items-center gap-1.5">
          <XCircle size={14} /> Vous ne pourrez pas être présent
        </div>
        <label className="block">
          <span className="text-[10px] text-[var(--color-ink-mid)] uppercase tracking-wider font-bold">
            Raison (optionnelle)
          </span>
          <textarea
            value={declineNote}
            ...
            className="mt-1 w-full text-[13px] rounded-lg border border-[var(--color-sand-border)] bg-[var(--color-cream)] text-[var(--color-ink)] px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[var(--color-navy)]/30"
          />
          <span className="text-[10px] text-[var(--color-ink-mid)] block text-right mt-0.5">
        ...
        <div className="flex gap-2">
          <button
            ...
            onClick={resetAll}
            disabled={pending}
            className="flex-1 bg-[var(--color-cream)] border border-[var(--color-sand-border)] text-[var(--color-ink-mid)] py-2.5 rounded-xl font-bold text-[13px] disabled:opacity-50"
          >
            Annuler
          </button>
          <button
            ...
            disabled={pending}
            className="flex-1 bg-[var(--color-terra)] text-[var(--color-cream)] py-2.5 rounded-xl font-bold text-[13px] disabled:opacity-50"
          >
            {pending ? 'Envoi…' : 'Envoyer ma réponse'}
          </button>
        </div>
        {error && (
          <div className="text-[12px] text-[var(--color-terra)] bg-[var(--color-terra-light)] border border-[var(--color-terra-mid)] rounded-md px-3 py-2">
            {error}
          </div>
        )}
      </div>
    ...
    <div className="bg-[var(--color-sand-mid)] border border-[var(--color-sand-border)] rounded-xl p-3.5 space-y-2.5">
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          disabled={pending}
          className="flex-1 bg-[var(--color-navy)] text-[var(--color-cream)] py-2.5 rounded-xl font-bold text-[13px] disabled:opacity-50"
        >
