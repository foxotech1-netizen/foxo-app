...
export async function POST(request: Request) {
  ...
  // 2a. Tarif depuis parametres (clé 'tarif_intervention' — string),
  //     fallback 150.00 si absent ou non parseable.
  ...
  // 2c. Totaux (1 ligne, pas de remise)
  const tvaPct = 21;
  const ht = Math.round(prixUnitaire * 100) / 100;
  const tva = Math.round(prixUnitaire * tvaPct) / 100;
  const ttc = Math.round((ht + tva) * 100) / 100;

  // 2d. Dates (émission = aujourd'hui, échéance = +15 jours par défaut)
